...
const SuperTokensHelpers = {
  ...
  async emailExists(email: string): Promise<boolean> {
    return SuperTokens.listUsersByAccountInfo(tenantId, { email }).then(
  ...
  async refreshToken({
    ...
    try {
      ...
      if (error.payload.sessionHandle)
        Session.revokeSession(error.payload.sessionHandle);
      return {};
    }
  },
  ...
  async sendPasswordResetEmail(email: string): Promise<void> {
    ...
    if (!user)
      return console.log(
        `Password reset email not sent to unrecognized address: ${email}`
      );
    ...
    if (tokenResult.status === 'UNKNOWN_USER_ID_ERROR') {
      return console.log(
        `Password reset email not sent, unknown user id: ${user.id}`
      );
    }
  ...
  async resetPassword(
    ...
    const { status } = await EmailPassword.resetPasswordUsingToken(
      tenantId,
      token,
      newPassword
    );
