...
export function createCookieSettings(
  type?: keyof typeof authCookieNames
): CookieSettings {
  ...
  return {
    expires: nextYear,
    path: type === 'refresh' ? commonRoutes.refreshSession : '/',
    ...commonCookieSettings,
  };
}

const deleteCookieSettings = Object.freeze({
  expires: new Date(0),
  path: '/',
} as const satisfies CookieSettings);
const deleteRefreshSettings = Object.freeze({
  ...deleteCookieSettings,
  path: commonRoutes.refreshSession,
});
